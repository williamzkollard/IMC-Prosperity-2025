...
class Trader:
    ...
    def __init__(self):
        ...
        self.kelp = []
        self.ewma_short_period = 500
        self.ewma_long_period = 1000
    ...
    def _get_orders_kelp_ewma(self, state: TradingState):
        df = pd.DataFrame.from_records(self.kelp)
        if len(df) < self.ewma_long_period + self.smoothing_window:
            return []

        df["short_ewma"] = df["mid_price_vw"].ewm(span=self.ewma_short_period).mean()
        df["long_ewma"] = df["mid_price_vw"].ewm(span=self.ewma_long_period).mean()

        macd = df["short_ewma"] - df["long_ewma"]
        signal = macd.ewm(span=self.smoothing_window, adjust=False).mean()

        current_pos = state.position.get("KELP", 0)
        best_bid = df["bid_price_1"].iloc[-1]
        best_ask = df["ask_price_1"].iloc[-1]

        orders = []
        if macd.iloc[-2] < signal.iloc[-2] and macd.iloc[-1] > signal.iloc[-1]:
            # Bullish crossover → Buy
            bid_price = best_ask - 1
            orders.append(Order("KELP", int(bid_price), min(20 - current_pos, 5)))
        elif macd.iloc[-2] > signal.iloc[-2] and macd.iloc[-1] < signal.iloc[-1]:
            # Bearish crossover → Sell
            ask_price = best_bid + 1
            orders.append(Order("KELP", int(ask_price), -min(20 + current_pos, 5)))
    ...
    def run(self, state: TradingState):
        ...
        # get orders for KELP
        order_kelp = self._get_orders_kelp_ewma(state)
        if order_kelp:
