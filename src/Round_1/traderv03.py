...
PARAMS = {
    ...
    Product.KELP: {
        ...
        "reversion_beta": 0.008,  # Mean reversion parameter
        "disregard_edge": 1,
        "join_edge": 0,
        "default_edge": 1,
    },
    Product.SQUID_INK: {
        ...
        "prevent_adverse": True,  # Avoid large trades that could move the market against us
        "adverse_volume": 15,  # Maximum volume to avoid for adverse selection
        "reversion_beta": 0.055,  # Mean reversion parameter
        "disregard_edge": 1,
        "join_edge": 0,
        "default_edge": 1,
    },
...
class Trader:
    ...
    def kelp_fair_value(self, order_depth: OrderDepth, traderObject, state) -> float:

        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            best_ask = min(order_depth.sell_orders.keys())
            best_bid = max(order_depth.buy_orders.keys())

            # Filter out small-volume noise
            filtered_ask = [
                price
            ...
            # --- ARIMA-based fair value logic ---
            price_history = traderObject.get("kelp_price_history", [])
            price_history.append(mmmid_price)
            if len(price_history) > 60:
                price_history.pop(0)

            if len(price_history) >= 3 and state.timestamp <= 400000:
                # Manual ARIMA(1,1,0) prediction
                delta_t = price_history[-1] - price_history[-2]
                delta_t_minus_1 = price_history[-2] - price_history[-3]

                # Estimate phi (AR(1) coefficient)
                if delta_t_minus_1 != 0:
                    phi = delta_t / delta_t_minus_1
                else:
                    phi = 0  # fallback to avoid divide-by-zero

                predicted_delta = phi * delta_t
                fair = price_history[-1] + predicted_delta
            else:
                # Not enough history — just use raw mid
                fair = mmmid_price
    ...
    def run(self, state: TradingState):
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            kelp_fair_value = self.kelp_fair_value(
                state.order_depths[Product.KELP], traderObject, state
            )
            kelp_take_orders, buy_order_volume, sell_order_volume = (
                self.take_orders(
                    Product.KELP,
                    state.order_depths[Product.KELP],
                    kelp_fair_value, #passes the function 
