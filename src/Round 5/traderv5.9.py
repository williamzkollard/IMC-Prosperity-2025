...
class Trader:
    ...
    Product.VOLCANIC_ROCK_VOUCHER_10500,
        ...
        for trade in traderObject.get("open_spread_trades", []):
            ...
            primary = trade["primary_leg"]
            if primary == over:
                primary_diff = iv_over - fitted_iv_over
            else:
                primary_diff = iv_under - fitted_iv_under

            if abs(primary_diff) < exit_threshold:
        ...
        if leg_to_buy and leg_to_sell:
            ...
            if take_orders or make_orders:
                ...
                traderObject["open_spread_trades"].append({
                    ...
                    "primary_leg": leg_to_sell["voucher"] if abs(leg_to_sell["diff"]) > abs(leg_to_buy["diff"]) else leg_to_buy["voucher"]
                })
                ...
                hedge_orders = self.volcanic_rock_delta_hedge_orders(
                    ...
                    take_orders,
                    volcanic_position,
                    0,  # optional placeholder; as not used in net-delta hedging
                    net_delta,
                )
