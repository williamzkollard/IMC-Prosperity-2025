...
class BlackScholes:
    ...
    def vega(spot: float, strike: float, tte: float, vol: float) -> float:
        ...
        return spot * statistics.NormalDist().pdf(d1) * math.sqrt(tte)
...
class Trader:
    ...
    def volcanic_rock_vol_spread_orders(
        ...
        state: TradingState,
    ) -> Tuple[List[Order], List[Order], int, int]:
        ...
        timestamp_years = state.timestamp / 1_000_000 / 365

        od_over = order_depths[overpriced_voucher]
        od_under = order_depths[underpriced_voucher]

        pos_over = positions.get(overpriced_voucher, 0)
        pos_under = positions.get(underpriced_voucher, 0)

        strike_over = self.params[overpriced_voucher]["strike"]
        strike_under = self.params[underpriced_voucher]["strike"]
        
        tte_over = self.params[overpriced_voucher]["starting_time_to_expiry"] - timestamp_years
        tte_under = self.params[underpriced_voucher]["starting_time_to_expiry"] - timestamp_years

        # Mid price of the underlying
        underlying_od = order_depths[Product.VOLCANIC_ROCK]
        spot = (max(underlying_od.buy_orders.keys()) + min(underlying_od.sell_orders.keys())) / 2

        # Use base IV
        base_iv = np.mean(traderObject["base_iv_data"])

        # --- Vega calculation ---
        vega_over = BlackScholes.vega(spot, strike_over, tte_over, base_iv)
        vega_under = BlackScholes.vega(spot, strike_under, tte_under, base_iv)

        if vega_under == 0 or vega_over == 0:
            return [], [], 0, 0  # avoid divide by zero

        # Determine the raw hedge ratio
        hedge_ratio = vega_over / vega_under

        # Round hedge ratio to nearest tradable integer
        hedge_qty = max(1, int(round(hedge_ratio)))

        # --- Get max tradable quantities ---
        limit_over = self.LIMIT[overpriced_voucher]
        limit_under = self.LIMIT[underpriced_voucher]

        max_qty_over = min(abs(-limit_over - pos_over), self.LIMIT[overpriced_voucher])
        max_qty_under = min(abs(limit_under - pos_under), self.LIMIT[underpriced_voucher])

        # Determine how much we can do while staying within both limits
        qty_over = min(max_qty_over, int(max_qty_under / hedge_qty))

        if qty_over <= 0:
            return [], [], 0, 0

        qty_under = hedge_qty * qty_over

        # Sell overpriced leg
        if od_over.buy_orders:
            best_bid = max(od_over.buy_orders.keys())
            take_orders.append(Order(overpriced_voucher, best_bid, -qty_over))

        # Buy underpriced leg
        if od_under.sell_orders:
            best_ask = min(od_under.sell_orders.keys())
            take_orders.append(Order(underpriced_voucher, best_ask, qty_under))


        return take_orders, make_orders, qty_over, qty_under
    ...
    Product.VOLCANIC_ROCK_VOUCHER_10500,
        ...
        if leg_to_buy and leg_to_sell:
            take_orders, make_orders, qty_over, qty_under = self.volcanic_rock_vol_spread_orders(
                leg_to_sell["voucher"],
                leg_to_buy["voucher"],
                state.order_depths,
                state.position,
            ...
            if take_orders or make_orders:
                ...
                net_delta = (-qty_over * delta_sell) + (qty_under * delta_buy)
