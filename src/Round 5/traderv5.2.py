...
class Trader:
    ...
    def volcanic_rock_voucher_orders(
        ...
        elif abs_diff <= -self.params[product]["diff_threshold"]:
            if voucher_position != self.LIMIT[product]:
                target_voucher_position = self.LIMIT[product]
    ...
    def run(self, state: TradingState):
        ...
        # Global IV data across strikes for fitting
        strike_iv_data = {}

        # Step 1: Gather strike-to-IV data
        for voucher in volcanic_vouchers:
        ...
        # Step 2: Fit the smile curve if enough strikes
        fitted_coeffs = None
        base_iv = None
        ...
        # Step 3: Evaluate mispricings and trade
        for voucher in volcanic_vouchers:
            if voucher in self.params and voucher in state.order_depths:
                strike = self.params[voucher]["strike"]
                tte = (
                ...
                iv = strike_iv_data.get(strike, None)
                ...
                if fitted_coeffs is not None:
                    ...
                    # No model to compare to - skip trading
                    continue
                ...
                #We need entry and exit logic of abs_diff here
                #This allows us to take positions in the over/underpriced and corresponding hedge 
                #We then need to monitor the abs_diff for the over/underpriced option and close out position when suitable
                #Remove the z-score for the baskets...





                # Your existing order generation logic
