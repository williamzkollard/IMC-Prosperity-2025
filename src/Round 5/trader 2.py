...
class BlackScholes:
    ...
    @staticmethod
    def implied_volatility(call_price, spot, strike, time_to_expiry, max_iterations=200, tolerance=1e-1):
        low_vol = 0.01
        high_vol = 1.0
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        option_prices = np.zeros(5)
        strikes = np.array([9500, 9750, 10000, 10250, 10500])
        IVS = np.zeros(5)
        positions = np.zeros(5)
        
        
        positions[0] = state.position.get(volcanic_vouchers[0], 0)
        positions[1] = state.position.get(volcanic_vouchers[1], 0)
        positions[2] = state.position.get(volcanic_vouchers[2], 0)
        positions[3] = state.position.get(volcanic_vouchers[3], 0)
        positions[4] = state.position.get(volcanic_vouchers[4], 0)
        ...
        for i, voucher in enumerate(volcanic_vouchers):
            result[voucher] = []
            ...
            if voucher in self.params and voucher in state.order_depths:
                ...
                option_prices[i] = self.get_volcanic_rock_voucher_mid_price(  
                    voucher_order_depth,
                    traderObject[voucher],
                )
                ...
                delta[i] = BlackScholes.delta(volcanic_mid_price, strikes[i], tte, IVS[i])
        
        m_t = np.log(strikes/option_prices)
        ...
        if 0 not in IVS:
            ...
            X = np.vstack([x, x2, ones]).T
            y= IVS.reshape((-1,1))
            theta = np.linalg.inv(X.T @ X) @ (X.T @ y)
            y_fit = X @ theta
            diff = (y - y_fit).reshape((5,-1))
            for i, voucher in enumerate(volcanic_vouchers):
                
                if diff[i]<-threshold:
                    if state.order_depths[voucher].sell_orders.keys():
                        price = min(state.order_depths[voucher].sell_orders.keys())
                    
                        quantity = min(200 - positions[i],
                                       abs(state.order_depths[voucher].sell_orders[price]))
                        result[voucher]+=[Order(voucher, price, int(quantity))]
                        positions[i]+=quantity
                        
                
                elif diff[i]>threshold:
                    if state.order_depths[voucher].buy_orders.keys():
                        price = max(state.order_depths[voucher].buy_orders.keys())
                        quantity = min(positions[i] + 200, 
                                       abs(state.order_depths[voucher].buy_orders[price]))
                        result[voucher]+=[Order(voucher, price, int(-quantity))]
                        positions[i]-=quantity
                        
                elif abs(diff[i])<threshold/2: 
                    if positions[i]>0:
                        if state.order_depths[voucher].buy_orders.keys():
                            price = max(state.order_depths[voucher].buy_orders.keys())
                            quantity = min(positions[i], abs(state.order_depths[voucher].buy_orders[price]))
                            result[voucher]+=[Order(voucher, price, int(-quantity))]
                            positions[i]-=quantity
                    elif positions[i]<0:
                        if state.order_depths[voucher].sell_orders.keys():
                            price = min(state.order_depths[voucher].sell_orders.keys())
                            quantity = min(-positions[i], abs(state.order_depths[voucher].sell_orders[price]))
                            result[voucher]+=[Order(voucher, price, int(quantity))]
                            positions[i]+=quantity
            delta_acc = (positions*delta).sum()
            if delta_acc>0:
                price = max(state.order_depths['VOLCANIC_ROCK'].buy_orders.keys())
                quantity = min(delta_acc, volcanic_position+400)
                result['VOLCANIC_ROCK']+=[Order(voucher, price, -int(quantity))]
            else:
                price = min(state.order_depths['VOLCANIC_ROCK'].sell_orders.keys())
                quantity = min(-delta_acc, 400-volcanic_position)
                result['VOLCANIC_ROCK']+=[Order(voucher, price, int(quantity))]
