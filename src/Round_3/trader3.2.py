...
PARAMS = {
    ...
    Product.VOLCANIC_ROCK_VOUCHER_10000: {
        "mean_volatility": 0.47, #calculate from data
        "threshold": 0.00163, #unsure? 
        "strike": 10000,
        "starting_time_to_expiry": 5 / 250, #recompute each round
        "std_window": 6, #calculate from data?
        "zscore_threshold": 7, #calculate from data
    },
   ...
   ####################################################################
        ...
        for voucher in volcanic_vouchers:
            ...
            if voucher in self.params and voucher in state.order_depths:
                ...
                volcanic_mid_price = (
                    min(volcanic_order_depth.buy_orders.keys())
                    + max(volcanic_order_depth.sell_orders.keys())
                ) / 2
