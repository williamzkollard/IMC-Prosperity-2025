...
class Trader:
    ...
    def _stoikov_bidask(
        ...
        ref_price = mid_price - q * gamma * sigma ** 2 
        stoikov_spread = gamma * sigma ** 2 + 2 / gamma * np.log(1 + gamma / k)
    ...
    def squid_ink_fair_value(self, order_depth: OrderDepth, traderObject, market_trades=None) -> float:
       ...
       # self.squidink.append(mid_price) #use to calculate the rolling volatility 

        if "squidink_prices" not in traderObject:
            traderObject["squidink_prices"] = []

        if mid_price is not None and math.isfinite(mid_price):
            traderObject["squidink_prices"].append(mid_price)

         # Keep it from growing forever
        if len(traderObject["squidink_prices"]) > 200:
            traderObject["squidink_prices"].pop(0)
    ...
    def make_orders(
        ...
        if manage_position:
            ...
            if product == Product.SQUID_INK:
                ...
                squidink_prices = pd.DataFrame(traderObject.get("squidink_prices", []), columns=["mid_price"])


                rolling_var = squidink_prices.rolling(10, min_periods=5).var()
                if not rolling_var.empty:
                    sigma = float(rolling_var["mid_price"].iloc[-1])
                    if math.isnan(sigma):
                        sigma = 2
                else:
                    sigma = 2
                ...
                k = 0.5
                bid, ask = self._stoikov_bidask(mid_price, current_pos, target_pos, gamma, sigma, k)
