...
class Trader:
    ...
    def get_synthetic_basket_order_depth(
        ...
        # Constants
        CROISSANT_PER_BASKET = BASKET_WEIGHTS[Product.CROISSANTS]
        JAM_PER_BASKET = BASKET_WEIGHTS[Product.JAMS]
        DJEMBE_PER_BASKET = BASKET_WEIGHTS[Product.DJEMBES]
        ...
        if implied_bid > 0:
            ...
            synthetic_order_price.buy_orders[implied_bid] = bid_vol
        ...
        if implied_ask < float("inf"):
            ...
            synthetic_order_price.sell_orders[implied_ask] = -ask_vol
    ...
    def convert_synthetic_basket_orders(self, synthetic_orders: List[Order], order_depths: Dict[str, OrderDepth]) -> Dict[str, List[Order]]:
        ...
        for order in synthetic_orders:
            ...
            croissant_order = Order(Product.CROISSANTS, croissant_price, quantity * BASKET_WEIGHTS[Product.CROISSANTS])
            jam_order = Order(Product.JAMS, jam_price, quantity * BASKET_WEIGHTS[Product.JAMS])
            djember_order = Order(Product.DJEMBES, djembe_price, quantity * BASKET_WEIGHTS[Product.DJEMBES])
    ...
    def execute_spread_orders_basket(
        self,
        target_position: int,
        basket_position: int,
    ...
    def spread_orders(
        ...
        if zscore >= self.params[Product.SPREAD]["zscore_threshold"]:
            if basket_position != -self.params[Product.SPREAD]["target_position"]:
                return self.execute_spread_orders_basket(
